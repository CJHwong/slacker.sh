...
#   SLACKER_SKIP_LIVE=1 ./.dev/tests/run.sh   offline only (no token needed)
#   ./.dev/tests/unit.sh                      pure functions + render.jq, directly
#   ./.dev/tests/actions.sh                   offline end-to-end, directly
...
unit_tests
action_tests
# The token here is the caller's, from the environment or .env — the per-case
# STUB_TOKEN overrides inside action_tests are scoped to their own subshells.
